...
void drawSensors() {
  ...
  SendMqtt(mqtt_temp, dispTemp);
  lcd.write(223);
  lcd.setCursor(6, 2);
  lcd.print(" " + String(dispHum) + "%  ");
  SendMqtt(mqtt_hum, dispHum);

  #if (CO2_SENSOR == 1)
    lcd.print(String(dispCO2) + " ppm");
    if (dispCO2 < 1000) lcd.print(" ");
    SendMqtt(mqtt_ppm, dispCO2);
  #endif
    ...
    SendMqtt(mqtt_status, "1");
  
#else
  // дисплей 1602
  lcd.setCursor(0, 0);
  lcd.print(String(dispTemp, 1));
  mqttClient.publish(mqtt_temp, MQTT_QOS, false, String(dispTemp));
  lcd.write(223);
  lcd.setCursor(6, 0);
  lcd.print(String(dispHum) + "% ");
  SendMqtt(mqtt_hum, dispHum);
